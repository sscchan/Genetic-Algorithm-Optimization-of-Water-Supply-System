...
/*************************** END OF EPANET.C ***************************/
 ...
 int DLLEXPORT ENgetNumberOfCurves()
 {
	 int numberOfCurves;
	 ENgetcount(EN_CURVECOUNT, &numberOfCurves);
	 return numberOfCurves;
 }

 int DLLEXPORT ENsetPumpHeadFlowCurve(int linkIndex, int curveIndex){

	 int iCounter;

//	Check for invalid calls
	 assert(Link[linkIndex].Type == PUMP);
	 assert(linkIndex <= Nlinks);
	
	 assert(curveIndex <= Ncurves);
	 //assert(Curve[curveIndex].Type == P_CURVE);

	
	for (iCounter = 1; iCounter <= Npumps; iCounter++){
		if (Pump[iCounter].Link == linkIndex)
			Pump[iCounter].Hcurve = curveIndex;
	}

	return 0;
 }

 int DLLEXPORT ENsetPumpEffeciencyFlowCurve(int linkIndex, int curveIndex){
	 ...
	 assert(curveIndex <= Ncurves);
	 //assert(Curve[curveIndex].Type == E_CURVE);


	 for (iCounter = 1; iCounter <=Npumps; iCounter++){
		 if (Pump[iCounter].Link == linkIndex)
			 Pump[iCounter].Ecurve = curveIndex;
	 }
	 return 0;
 }

 double DLLEXPORT ENgetPumpCostPerDay(int linkIndex){
	 ...
	 for (iCounter = 1; iCounter <= Npumps; iCounter++){
		 if (Pump[iCounter].Link == linkIndex)
			 return Pump[iCounter].Energy[5];
	 }
	 return 0;
 }

 // Returns averages kw
 double DLLEXPORT ENgetPumpEnergyUsage(int linkIndex){
	 int i;
	 ...
	 for (i = 1; i <= Npumps; i++){
		 if (Pump[i].Link == linkIndex)
			 return Pump[i].Energy[3];
	 }
 ...
 void DLLEXPORT ENgetCurveID(int curveIndex, char* bufferID){
	 assert(curveIndex <= Ncurves);
	 assert(curveIndex > 0);
	 strcpy(bufferID,Curve[curveIndex].ID);
 }